...
class SimpleCache {
    ...
    invalidateProducts(): void {
        this.clearPattern('products');
        this.clearPattern('categories');
        console.log('🗑️ Cache de productos invalidado');
    }

    invalidateTransactions(): void {
        this.clearPattern('transactions');
        this.clearPattern('transaction_products');
        console.log('🗑️ Cache de transacciones invalidado');
    }
}
...
export const productApi = {
    ...
    create: async (product: CreateProductRequest): Promise<Product> => {
        try {
            ...
            NotificationService.show(`Producto "${product.name}" creado exitosamente`, 'success');

            console.log('✅ Producto creado, cache de productos invalidado');
            return result.data!;
        } catch (error) {
            const message = error instanceof Error ? error.message : 'Error desconocido';
            NotificationService.show(`Error: ${message}`, 'danger');
    ...
    update: async (id: number, product: CreateProductRequest): Promise<Product> => {
        try {
            ...
            NotificationService.show(`Producto "${product.name}" actualizado exitosamente`, 'success');

            console.log('✅ Producto actualizado, cache invalidado');
            return result.data!;
        } catch (error) {
            const message = error instanceof Error ? error.message : 'Error desconocido';
            NotificationService.show(`Error: ${message}`, 'danger');
    ...
    delete: async (id: number): Promise<boolean> => {
        try {
            ...
            console.log('✅ Producto eliminado, cache invalidado');
            return true;
        } catch (error) {
            const message = error instanceof Error ? error.message : 'Error desconocido';
            NotificationService.show(`Error: ${message}`, 'danger');
...
export const transactionApi = {
    ...
    create: async (transaction: CreateTransactionRequest): Promise<Transaction> => {
        try {
            ...
            const tipoTexto = transaction.transactionType === 'Purchase' ? 'Compra' : 'Venta';
            NotificationService.show(
                `${tipoTexto} registrada: ${transaction.quantity} unidades`,
                'success'
            );

            console.log('✅ Transacción creada, cache invalidado');
            return result.data!;
        } catch (error) {
            const message = error instanceof Error ? error.message : 'Error desconocido';
            NotificationService.show(`Error: ${message}`, 'danger');
    ...
    update: async (id: number, transaction: UpdateTransactionRequest): Promise<Transaction> => {
        try {
            ...
            console.log('✅ Transacción actualizada, cache invalidado');
            return result.data!;
        } catch (error) {
            const message = error instanceof Error ? error.message : 'Error desconocido';
            NotificationService.show(`Error: ${message}`, 'danger');
    ...
    delete: async (id: number): Promise<boolean> => {
        try {
            ...
            console.log('✅ Transacción eliminada, cache invalidado');
            return true;
        } catch (error) {
            const message = error instanceof Error ? error.message : 'Error desconocido';
            NotificationService.show(`Error: ${message}`, 'danger');
    ...
    getProducts: async (forceRefresh: boolean = false): Promise<Product[]> => {
        try {
            ...
            if (forceRefresh) {
                cache.clearPattern('transaction_products');
            }
